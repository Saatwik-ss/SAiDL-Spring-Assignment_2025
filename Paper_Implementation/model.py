...
# Board size (extra space on right for move history)
...
pygame.display.set_caption("Connect Four with AI (Second Mover)")
...
class GameBoard:
    ...
    def draw_board(board, move_list):
        flipped = np.flipud(board)  # Flip for correct visualization
        screen.fill(black)
...
# --- Model Setup for AI (Second Mover) ---
...
model.load_state_dict(torch.load(r"C:\Users\saatw\connect_four_model.pt", map_location=device))
...
def ai_choose_move(board):
    ...
    """
    valid_moves = [c for c in range(column_count) if GameBoard.is_valid_location(board, c)]
    best_move = None
    ...
    for col in valid_moves:
        ...
        if row is None:
            continue
        # simulate board: copy and drop AI's piece (which is 2 in the game board)
...
move_list = []  # Stores moves (e.g., "P1: A1")
...
while not game_over:
    for event in pygame.event.get():
        ...
        # Human move: Only allow mouse events if it's player's turn (turn==0)
        if turn == 0:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if col < column_count and GameBoard.is_valid_location(board, col):
                    ...
                    if GameBoard.winning_move(board, 1):
        ...
        if turn == 1 and not game_over:
            # Give a brief pause so the human sees the move change
            time.sleep(1)
            col = ai_choose_move(board)
            if col is not None and GameBoard.is_valid_location(board, col):
                row = GameBoard.get_next_open_row(board, col)
                GameBoard.drop_piece(board, row, col, 2)  # AI piece
                move_notation = get_position_label(row, col)
                move_list.append(f"P2: {move_notation}")
                
                if GameBoard.winning_move(board, 2):
