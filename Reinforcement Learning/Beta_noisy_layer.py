...
class NoisyLinearBeta(nn.Module):
    ...
    def reset_noise(self):
        eps_in = self._scale_noise_beta(self.in_features)
        eps_out = self._scale_noise_beta(self.out_features)
    ...
    @staticmethod
    def _scale_noise_beta(size, alpha=2.0, beta_val=2.0):
        beta_dist = torch.distributions.Beta(torch.tensor([alpha]), torch.tensor([beta_val]))
        x = beta_dist.sample((size,)).squeeze()
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        nn.Module.__init__(self)
        self.noisy1 = NoisyLinearBeta(state_dim, 256)
        self.noisy2 = NoisyLinearBeta(256, 256)
        self.noisy3 = NoisyLinearBeta(256, action_dim)
